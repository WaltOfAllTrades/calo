...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const keyDefs = [
        ...
        { display: 'CE', value: 'CE' }, { display: '0', value: '0' }, { display: 'svg', value: '✓' }
    ];

    keyDefs.forEach(({ display, value }) => {
        const button = document.createElement('button');
        button.className = 'key-button';
        if (value === 'CE') button.className += ' operator';
        ...
        button.addEventListener('click', () => {
            if (value === 'CE') {
                // Clear the input field by dispatching a special event
                const event = new CustomEvent('keypadPress', { detail: { key: 'CE' } });
